...
import { MdFileDownload, MdFolderZip } from "react-icons/md";
...
const MessageContainer = () => {
  ...
  const downloadFile = async (url) => {
    ...
    try {
      ...
      link.href = urlBlob;
      // fixing download filename extraction
      link.setAttribute("download", url.split("/").pop());
      document.body.appendChild(link);
      link.click();
      link.remove();
      // to avoid memory leaks ill revoke the object URL
        ...
        >
          {checkIfImage(message.fileUrl) ? (
            ...
            <div className="flex items-center justify-between gap-2 max-w-full">
              <span className="text-white/80 text-3xl bg-black/20 rounded-full p-2 flex-shrink-0">
                <MdFolderZip />
              </span>
        ...
        <div className="fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center backdrop-blur-lg flex-col">
          ...
          <div className="flex gap-5 fixed top-0 mt-5">
            <button className="bg-black/20 p-3 text-2xl rounded-full hover:bg-black/50 cursor-pointer transition-all duration-300">
              <MdFileDownload onClick={() => downloadFile(imageURL)} />
            </button>
            <button className="bg-black/20 p-3 text-2xl rounded-full hover:bg-black/50 cursor-pointer transition-all duration-300">
              <IoCloseSharp
                onClick={() => {
